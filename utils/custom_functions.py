...
class real_check(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        """
        ctx.save_for_backward(x)
        x1 = x.real.type(torch.cfloat)
        x2 = x.imag.type(torch.cfloat)
        return torch.add(torch.abs(x1), torch.mul(x2, 1j))
...
class imag_check(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ...
        x1 = x.real.type(torch.cfloat)
        x2 = x.imag.type(torch.cfloat)
        return torch.add(x1, torch.mul(torch.abs(x2), 1j))
